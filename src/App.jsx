...
const DEFAULT_PHOTOS = [
  { id: 1, title: "근조 3단", date: "2026-04-15", src: "" },
  { id: 2, title: "개업화분", date: "2026-04-14", src: "" },
  { id: 3, title: "축하 3단", date: "2026-04-13", src: "" },
  { id: 4, title: "몬스테라", date: "2026-04-12", src: "" },
];
...
function makeExpenseForm(type = "화환") {
  const items = {};
  EXPENSE_CATEGORIES[type].forEach((key) => {
    items[key] = "";
  });
  return {
    weekLabel: "2026년 4월 3주차",
...
function getMonthMatrix(year, monthIndex) {
  ...
  for (let i = 0; i < 42; i += 1) {
    ...
    if (dayNumber <= 0) {
      cells.push({ date: new Date(year, monthIndex - 1, prevMonthDays + dayNumber), currentMonth: false });
    } else if (dayNumber > daysInMonth) {
      cells.push({ date: new Date(year, monthIndex + 1, dayNumber - daysInMonth), currentMonth: false });
    } else {
      cells.push({ date: new Date(year, monthIndex, dayNumber), currentMonth: true });
    }
  }
...
function InputField({ label, value, onChange, type = "text", theme }) {
  return (
  ...
  return (
    <label style={{ display: "grid", gap: 8 }}>
      ...
      <textarea
        ...
        rows={4}
        style={{ ...theme.field, minHeight: 110, height: "auto", paddingTop: 14, resize: "vertical" }}
      />
    </label>
  );
}
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {filters.map((item) => (
          <button key={item} onClick={() => onChange(item)} style={active === item ? theme.filterActive : theme.filterButton}>
            {item}
          </button>
        ))}
      </div>
...
function DetailSection({ title, rows, formatter, theme }) {
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      {rows.length === 0 ? (
        <div style={{ color: theme.muted }}>없음</div>
      ) : (
        rows.map((row) => (
          <div key={row.id} style={theme.detailItem}>
            {formatter(row)}
          </div>
        ))
      )}
          ...
          { key: "chain", label: "체인사" },
          {
            ...
            render: (row) => (
              <>
                ...
                <div style={{ marginTop: 4, fontSize: 13, color: theme.muted }}>{row.type}</div>
              </>
            ),
          },
          { label: amountLabel, render: (row) => won(row.amount) },
...
const lightTheme = {
  ...
  activeTab: { border: "none", borderRadius: 14, padding: "12px 16px", background: "#111827", color: "#fff", fontWeight: 800, cursor: "pointer", whiteSpace: "nowrap" },
  ...
  subActiveTab: { border: "none", borderRadius: 14, padding: "12px 18px", background: "#111827", color: "#fff", fontWeight: 800, cursor: "pointer" },
  ...
  searchBox: { display: "flex", alignItems: "center", gap: 10, background: "#fff", border: "1px solid #e7ebf3", borderRadius: 18, padding: "12px 14px" },
  searchInput: { flex: 1, border: "none", outline: "none", background: "transparent", fontSize: 15, color: "#1f2937" },
  askButton: { border: "none", borderRadius: 12, padding: "10px 14px", background: "#111827", color: "#fff", fontWeight: 700, cursor: "pointer" },
  ...
  moreButton: { border: "1px solid #e7ebf3", background: "#fff", borderRadius: 12, padding: "10px 14px", cursor: "pointer", fontWeight: 700, color: "#334155" },
  uploadButton: { border: "1px solid #e7ebf3", background: "#fff", borderRadius: 12, padding: "10px 14px", cursor: "pointer", fontWeight: 700, color: "#334155" },
  ...
  filterButton: { border: "1px solid #e7ebf3", background: "#fff", color: "#475569", borderRadius: 12, padding: "10px 14px", fontWeight: 700, cursor: "pointer" },
  ...
  field: { width: "100%", border: "1px solid #e7ebf3", borderRadius: 14, padding: "0 14px", height: 48, background: "#fff", color: "#1f2937", outline: "none" },
  selectField: { width: "100%", border: "1px solid #e7ebf3", borderRadius: 14, padding: "0 14px", height: 48, background: "#fff", color: "#1f2937", outline: "none" },
  infoRowCard: { border: "1px solid #edf2f7", borderRadius: 20, padding: 18, background: "#fff" },
  ...
  rankBadge: { width: 32, height: 32, borderRadius: 999, display: "grid", placeItems: "center", background: "#111827", color: "#fff", fontWeight: 800 },
...
const darkTheme = {
  ...
  activeTab: { border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "12px 16px", background: "rgba(255,255,255,0.08)", color: "#fff", fontWeight: 800, cursor: "pointer", whiteSpace: "nowrap" },
  ...
  subActiveTab: { border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "12px 18px", background: "rgba(255,255,255,0.1)", color: "#fff", fontWeight: 800, cursor: "pointer" },
  ...
  askButton: { border: "none", borderRadius: 12, padding: "10px 14px", background: "rgba(255,255,255,0.1)", color: "#fff", fontWeight: 700, cursor: "pointer" },
  ...
  filterActive: { border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.12)", color: "#fff", borderRadius: 12, padding: "10px 14px", fontWeight: 800, cursor: "pointer" },
  ...
  selectField: { width: "100%", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "0 14px", height: 48, background: "rgba(255,255,255,0.03)", color: "#f8fafc", outline: "none" },
  ...
  rankBadge: { width: 32, height: 32, borderRadius: 999, display: "grid", placeItems: "center", background: "rgba(255,255,255,0.12)", color: "#fff", fontWeight: 800 },
...
export default function App() {
  ...
  const [orders, setOrders] = useState(() => loadLocal("bloomhub_orders", DEFAULT_ORDERS));
  const [purchases, setPurchases] = useState(() => loadLocal("bloomhub_purchases", DEFAULT_PURCHASES));
  const [partners, setPartners] = useState(() => loadLocal("bloomhub_partners", DEFAULT_PARTNERS));
  const [partnerSales, setPartnerSales] = useState(() => loadLocal("bloomhub_partner_sales", DEFAULT_PARTNER_SALES));
  const [weeklyExpenses, setWeeklyExpenses] = useState(() => loadLocal("bloomhub_weekly_expenses", DEFAULT_WEEKLY_EXPENSES));
  const [photos, setPhotos] = useState(() => loadLocal("bloomhub_photos", DEFAULT_PHOTOS));
  const [announcements, setAnnouncements] = useState(() => loadLocal("bloomhub_notices", DEFAULT_NOTICES));

  const [chainOptions, setChainOptions] = useState(() => loadLocal("bloomhub_chain_options", ["송죽플라워", "반하다플라워", "아이마플라워"]));
  const [floristOptions, setFloristOptions] = useState(() => loadLocal("bloomhub_florist_options", ["중앙화원", "그린화원", "새벽화원"]));
  const [productOptions, setProductOptions] = useState(() => loadLocal("bloomhub_product_options", ["근조 3단", "축하 3단", "개업화분", "몬스테라"]));
  const [partnerCategoryOptions, setPartnerCategoryOptions] = useState(() => loadLocal("bloomhub_partner_categories", ["화원", "기업"]));
  const [priceBandOptions, setPriceBandOptions] = useState(() => loadLocal("bloomhub_price_bands", ["10만원~20만원", "20만원 이상"]));
  const [partnerProductOptions, setPartnerProductOptions] = useState(() => loadLocal("bloomhub_partner_products", ["근조 3단", "개업화분"]));
  ...
  const [noticeForm, setNoticeForm] = useState({ title: "", date: getToday() });
  ...
  useEffect(() => saveLocal("bloomhub_orders", orders), [orders]);
  useEffect(() => saveLocal("bloomhub_purchases", purchases), [purchases]);
  useEffect(() => saveLocal("bloomhub_partners", partners), [partners]);
  useEffect(() => saveLocal("bloomhub_partner_sales", partnerSales), [partnerSales]);
  useEffect(() => saveLocal("bloomhub_weekly_expenses", weeklyExpenses), [weeklyExpenses]);
  useEffect(() => saveLocal("bloomhub_photos", photos), [photos]);
  useEffect(() => saveLocal("bloomhub_notices", announcements), [announcements]);

  useEffect(() => {
    saveLocal("bloomhub_chain_options", chainOptions);
    saveLocal("bloomhub_florist_options", floristOptions);
    saveLocal("bloomhub_product_options", productOptions);
    saveLocal("bloomhub_partner_categories", partnerCategoryOptions);
    saveLocal("bloomhub_price_bands", priceBandOptions);
    saveLocal("bloomhub_partner_products", partnerProductOptions);
  }, [chainOptions, floristOptions, productOptions, partnerCategoryOptions, priceBandOptions, partnerProductOptions]);

  const currentMonthSales = DEFAULT_MONTHLY_SALES[DEFAULT_MONTHLY_SALES.length - 1].sales;
  ...
  const maxSales = Math.max(...DEFAULT_MONTHLY_SALES.map((item) => item.sales));
  ...
  function saveSharedOptions(form) {
  ...
  function handlePartnerSubmit(e) {
    ...
    if (partnerForm.category && !partnerCategoryOptions.includes(partnerForm.category)) {
  ...
  function handlePartnerSaleSubmit(e) {
    e.preventDefault();
  ...
  function handleNoticeSubmit(e) {
    ...
    setAnnouncements((prev) => [{ id: Date.now(), ...noticeForm }, ...prev]);
    setNoticeForm({ title: "", date: getToday() });
  }

  function handleExpenseTypeChange(value) {
    setExpenseForm(makeExpenseForm(value));
  }

  function handleExpenseItemChange(key, value) {
      ...
      <div style={theme.windowWrap}>
        ...
        <div style={theme.body} className="bh-body">
              ...
              <div style={theme.heroCard}>
                <div>
                  ...
                  <div style={{ marginTop: 10, color: theme.muted, lineHeight: 1.6 }}>
                    이번달 매출, 지출, 최근 사진을 홈에서 바로 확인할 수 있어요.
                  </div>
                </div>
              ...
              <div className="bh-home-grid" style={{ display: "grid", gridTemplateColumns: "1.35fr 1fr", gap: 20 }}>
                ...
                <div style={theme.mainCard}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                    ...
                    <button style={theme.moreButton} onClick={() => setActiveTab("사진방")}>더보기</button>
                  </div>
                  <div className="bh-photo-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    {photos.slice(0, 4).map((photo) => (
                      <div key={photo.id} style={theme.photoCard}>
                        {photo.src ? <img src={photo.src} alt={photo.title} style={theme.photoImage} /> : <div style={theme.photoMock}>📸</div>}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {["전체", "수주", "발주", "거래처", "기타"].map((item) => (
                      <button key={item} onClick={() => setCalendarFilter(item)} style={calendarFilter === item ? theme.filterActive : theme.filterButton}>
                        {item}
                      </button>
                    ))}
                  </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderColor: selectedCalendarDate === iso ? theme.accent : theme.line,
                        }}
                      >
          ...
          {activeTab === "매출분석" && (
            <div style={{ display: "grid", gap: 20 }}>
              <div className="bh-summary-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
                <SummaryCard label="총 매출" value={won(orderSalesTotal + partnerSalesTotal)} theme={theme} />
                <SummaryCard label="총 지출" value={won(weeklyExpenseSummary.total)} theme={theme} />
                <SummaryCard label="순이익" value={won(orderSalesTotal + partnerSalesTotal - weeklyExpenseSummary.total)} accent theme={theme} />
              </div>
            ...
            <div className="bh-two-col" style={{ display: "grid", gridTemplateColumns: "420px 1fr", gap: 20 }}>
              <div style={theme.mainCard}>
                ...
                <form onSubmit={handleExpenseSubmit} style={{ display: "grid", gap: 12 }}>
                  ...
                  {EXPENSE_CATEGORIES[expenseForm.type].map((item) => (
                  ...
                  {filteredExpenses.map((row) => (
                    <div key={row.id} style={theme.infoRowCard}>
                      ...
                      <div style={{ marginTop: 12, color: theme.muted }}>
                        {Object.entries(row.items).filter(([, value]) => value > 0).map(([key]) => key).join(" + ") || "항목 없음"}
                      </div>
          ...
          {activeTab === "사진방" && (
            <div style={theme.mainCard}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, gap: 12, flexWrap: "wrap" }}>
                <div style={{ fontSize: 24, fontWeight: 800 }}>사진방</div>
                <label style={theme.uploadButton}>
                  사진 업로드
          ...
          {activeTab === "공지사항" && (
            <div className="bh-two-col" style={{ display: "grid", gridTemplateColumns: "420px 1fr", gap: 20 }}>
              <div style={theme.mainCard}>
                <SectionTitle title="공지 등록" />
                <form onSubmit={handleNoticeSubmit} style={{ display: "grid", gap: 12 }}>
                  <InputField label="공지 제목" value={noticeForm.title} onChange={(value) => setNoticeForm((prev) => ({ ...prev, title: value }))} theme={theme} />
                  <InputField label="날짜" type="date" value={noticeForm.date} onChange={(value) => setNoticeForm((prev) => ({ ...prev, date: value }))} theme={theme} />
                  <button type="submit" style={theme.primaryAction}>등록하기</button>
                </form>
              </div>
              <div style={theme.mainCard}>
